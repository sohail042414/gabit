 ...
 * @since 1.1
 */
class CoreCButtonColumn extends CGridColumn
{
    /**
...
EOD;
    ...
    protected function renderButton($id, $button, $row, $data)
    {
        ...
        // added logic for role base access by ketan barad
        $explode_url = explode('/', $url);
        $matched_key = '';
        if (is_array($explode_url) && !empty($explode_url)) {
            ...
            if (!empty($matched_key) && !empty($explode_url[$matched_key + 1]) && !empty($explode_url[$matched_key + 2])) {
                $route = $explode_url[$matched_key + 1] . '/' . $explode_url[$matched_key + 2];
                $check_permission = AdminCoreController::check_permission($route);
