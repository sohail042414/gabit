...
<div class="box-body" id="report_manage_fundraiser">
    <h3 style="margin-bottom:10px;">Case Updates (<?php echo count($case_updates_list); ?>)</h3>
    <table class="demo">
        <thead>
        <tr>
            <th>Fundraiser</th>
    ...
    <?php foreach($case_updates_list as $case){ ?>
        ...
        <td>
            <?php if(!empty($case->document1)){ ?>
                <a href="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case->document1; ?>" download>Document 1</a>
            <?php } ?> 
            <?php if(!empty($case->document2)){ ?>
                <br>
                <a target="_blank" href="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case->document2 ?>" >Document 2</a>
            <?php } ?> 
            <?php if(!empty($case->document3)){ ?>
                <br>
                <a target="_blank" href="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case->document3; ?>">Document 3</a>
            <?php } ?> 
        </td>
        <td>
            <a onclick="return confirm('Are you Sure you want to delete it case update? ');" href="<?php echo Yii::app()->createUrl('caseupdate/delete',array('case_id' => $case->id)); ?>"><span style="color:red;">Delete</span></a> <br>
...
<?php $form_heading = ($case_update->isNewRecord) ? 'New Case Update' : 'Edit Case Update'; ?>
<h3 style="margin: 10px 0px;"><?php echo $form_heading; ?></h3>
<?php
...
<div class="box-body">  
    ...
    <div class="form-group">
        ...
        ?>
        <?php echo $form->error($case_update, 'image_file'); ?>
    </div>
    <div class="form-group" >
    ...
    <?php if(!empty($case_update->document1)){ ?>
        <div class="form-group">
            ...
            <embed src="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case_update->document1; ?>" width="400px" height="200px" />
        </div>
    <?php } ?>
    <div class="form-group">
    ...
    <?php if(!empty($case_update->document2)){ ?>
        <div class="form-group">
            <p>Already Attached</p>
            <embed src="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case_update->document2; ?>" width="400px" height="200px" />
        </div>
    <?php } ?>
    <div class="form-group">
    ...
    <?php if(!empty($case_update->document3)){ ?>
        <div class="form-group">
            <p>Already Attached</p>
            <embed src="<?php echo SITE_ABS_PATH_CASE_UPDATES_DOCS.$case_update->document3; ?>" width="400px" height="200px" />
        </div>
    <?php } ?>
