...
class FundraiserController extends FrontCoreController
{
    ...
    public function actionIndex($id)
    {
        ...
        $fb_share_url = Yii::app()->createAbsoluteUrl("fundraiser/$id/$title/$user_slug");
    ...
    { 
        ...
        if(is_object($transaction) && isset($_REQUEST['status']) && ($_REQUEST['status'] == 'successful')){
            ...
            if(!is_object($existing)){
                ...
                //Referral code entry,
                //stopped it for now, it should happen if user completes donation. 
                /*
                if(!empty($donation->referral_code)){
                    $ref_user = Users::model()->findByReferralCode($donation->referral_code);
                    if(is_object($ref_user)){
                        //1 entry for referral code entry
                ...
                */
                ...
                if(!empty($donation->user_id) && empty($donation->referral_code)){
                                    
                    $donor_user = Users::model()->findByPk($donation->user_id);
                    if(is_object($donor_user)){

                        if(!empty($donor_user->referrer_id)){
                            $ref_user = Users::model()->findByReferralCode($donor_user->referrer_id);
                ...
                if(!empty($donation->referral_code)){
                    ...
                    if(is_object($ref_user) && ($ref_user->id != $donation->user_id)){
                        ...
                        if(empty($transaction->signup_check)){
                            $rewardModel->addPoints($transaction->fundraiser_id,'referral_code_entry',$donation->transaction_amount,$ref_user->id);
                        }
                    }
                }
    ...
    { 
        ...
        if(isset($post_data['event']) && $post_data['event'] == 'charge.completed'){
            ...
            if(is_object($transaction)){
                ...
                $message = 'Transaction Failed , Status: '.$_REQUEST['status'];
                $status = 'error';
            }
    ...
    public function actionFindfundraiser()
    {
        if (!empty($_REQUEST['fundraiser_id'])) {
            ...
            if($_REQUEST['fundraiser_id'] == 'all'){
                $featured_fundraiser = Fundraiser::model()->findAll(array('select' => '*', 'condition' => 'feature_flag = "Y" AND status = \'Y\' '));
            }else{
                $featured_fundraiser = Fundraiser::model()->findAll(array('select' => '*', 'condition' => 'feature_flag = "Y" AND status = \'Y\' AND ftype_id = "' . $_REQUEST['fundraiser_id'] . '" AND status = "Y" '));
            }
