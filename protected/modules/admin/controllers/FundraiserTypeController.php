...
class FundraiserTypeController extends AdminCoreController {

    /**
     * Do not change Resource Id value, it can cause permissions out of order. 
     */
    public $resource_id = 9;
	...
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'FundraiserType'),
		));
	}

	public function actionCreate() {
		$model = new FundraiserType;


		if (isset($_POST['FundraiserType'])) {

			$model->setAttributes($_POST['FundraiserType']);
            $model->type_description  = $_POST['FundraiserType']['type_description'];
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
			$this->loadModel($id, 'FundraiserType')->delete();
	...
	public function actionAdmin() {
		...
		if (isset($_GET['FundraiserType']))
			$model->setAttributes($_GET['FundraiserType']);
