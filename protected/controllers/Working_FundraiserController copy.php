...
class FundraiserController extends FrontCoreController
{
    ...
    public function actionTestpayment(){
        $this->layout = 'main';
        $this->render('test-payment', array(
        ));
    }
    ...
    public function actionDonations($id)
    {
        ...
        //$fundraiser = SetupFundraiser::model()->findByPk($id);
        //ALTER TABLE `donations` ADD `trans_ref` VARCHAR(255) NULL DEFAULT NULL AFTER `status`;
        ...
        if (isset($_POST['Transactions'])) {        
            ...
            $model->user_id = isset($_SESSION['front_id']) ? $_SESSION['front_id'] : 0;
            $model->donor_message = '--';
            $model->status = 'N';
            $model->trans_ref = $trxref;
            $model->fundraiser_id = $id;
            //$model->donor_phone_no = $_POST['Donations']['country_code']." ".$_POST['Donations']['donor_phone_no'];
            $model->save();

            $this->redirect(array('options', 
                    'id_value' => $model->id, 
                    'phone_no'=> $model->donor_phone_no, 
                    'fundraiser_id'=> $id, 
                    'donation_amount'=> $model->donation_amount, 
                    'donor_email'=> $model->donor_email, 
                    'donor_name'=> $model->donor_name, 
                    'user_id'=> isset($_SESSION['front_id']) ? $_SESSION['front_id'] : 0 , 
                    'country_code'=> $model->country_code, 
                    'donation_titl'=> $_REQUEST['fundraiser_name'],
                    'trxref' => $trxref
                )
            );
        }
    ...
    public function actionOptions()
    {
        ...
        $transaction_id  = $_REQUEST;

        $payment_url = Yii::app()->createUrl('fundraiser/payment',$donation_data);

        $merchant_code = '10578';
        $donation_messag = DonationMessage::model()->find(array('select' => 'messge'));
        $message_html = $donation_messag->messge;
        $message_html = str_replace('{donation_amount}',$donation_data['donation_amount'],$message_html);
        $message_html = str_replace('{merchant_code}',$merchant_code,$message_html);


        $this->render('options', array(
                //'model' => $model, 
                'fundraiser_id' => $donation_data['fundraiser_id'], 
                'fundraiser_name' => $_REQUEST['fundraiser_name'],
    ...
    public function actionPayment()
    {
        ...
        $donation_data = $_REQUEST;

        $model = new Donations;
        $model->setAttributes($donation_data);

        $return_url = Yii::app()->createAbsoluteUrl('fundraiser/donationcomplete');
        ...
        $this->render('payment', array(
            ...
            'trxref' => $donation_data['trxref'],
            'donor_name' => $donation_data['donor_name'] 
        ));
    }

    public function actionDonationcomplete()
    { 
        ...
        $donation = Donations::model()->find('trans_ref = :trans_ref',array(
            'trans_ref' => $trans_ref
        ));
        ...
        $this->render('donation_complete',array(
            'donation' => $donation,
    ...
    private function sendDonationEmail(){
